...
pmc_bg = readRDS('./data/bg_peak_mc_smoothed_mg.rds')

pmcn = rbind(pmc, pmc_bg)
pmcn_rn = as.data.frame(do.call('rbind', stringr::str_split(rownames(pmcn), '-')))
pmcn_rn[,2:3] = apply(pmcn_rn[,2:3], 2, as.numeric)
colnames(pmcn_rn) = c('chrom', 'start', 'end')

print(dim(pmc))
print(dim(pmc_bg))

pmc = pmcn[with(pmcn_rn, order(chrom, start)),]
print(dim(pmc))
...
pmc_inp = unique(pmc_inp)

print(dim(pmc_inp))
print(nrow(unique(pmc_inp)))
...
gtrack.rm(gtrack.ls('_tmp_200bp'), f=T)
...
create_intervals_pssm = function(trks) {
    ...
    sapply(seq_along(trks), function(trks, i) {
        ...
        # print(pssmid)
        trknm = paste0(pssmset, '.', trk_clean, '_tmp_200bp')
        trknm = gsub('-|\\$', '_', trknm)
...
trks_motifs = gtrack.ls('_tmp_200bp')
