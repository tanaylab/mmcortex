...
marks <- union(marks, unlist(mg_bon$marks))
...
hcl <- lapply(sort(unique(mark_fp_km$cluster)), function(u) hclust(tgs_dist(t(legc[marks,which(mark_fp_km$cluster == u)]))))
mok_hc_ord = unlist(sapply(seq_along(hcl), function(hci, i) which(mark_fp_km$cluster == i)[hci[[i]]$order], hci = hcl))
...
st[mark_fp_km$cluster == 4] = 'CPN_L5_6'
st[mark_fp_km$cluster == 5] = 'SCPN'
st[mark_fp_km$cluster == 6 & (is.na(st) | st != 'IPC')] = 'iCPN/CfuPN'
st[mark_fp_km$cluster == 6 & st == 'IPC'] = 'IPC_late'
st[mark_fp_km$cluster == 7 & st != 'IPC'] = 'iCPN_early'
st[mark_fp_km$cluster == 7 & st == 'IPC'] = 'IPC_late'
st[mark_fp_km$cluster == 8 & is.na(st)] <- "iCPN_late"
st[mark_fp_km$cluster == 9 & is.na(st)] <- "iCPN_late"
st[mark_fp_km$cluster == 10 & is.na(st)] <- "iCPN_early"
