...
st[st == 'Astrocytes' & st_mark_mc$Astrocytes < 15 & apply(day_mat[,c('E13','E14','E15')], 1, function(x) any(x != 0))] <- 'NSC'
...
mcell_mc2d_plot(nm, colors = mc@colors, fig_fn = mc2d_path)
mc2 <- mc
...
mcmd_new$mean_day = 12+rowSums(day_mat*(1:ncol(day_mat)))/rowSums(day_mat)
