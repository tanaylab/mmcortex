...
my_mctnetflow_plot_gene = function(mct_id, gene, fn,
						mc_ord = NULL, 
                        clust_vec = NULL,
						filt_mcs = NULL,
                        colors_ordered = NULL,
						...
						score_shades = colorRampPalette(c("lightgray", "gray", "darkgray", "lightpink", "pink", "red", "darkred"))(1000))
{
    ...
    mct@network$flow = mcf@edge_flows
	if(is.null(mct)) {
		stop("cannot find mctnet object ", mct_id, " when trying to plot net flows")
	}
	net= mct@network
	...
	if (!(g %in% rownames(mc@e_gc))) {
        stop("cannot find gene ", g, " in e_gc of mc ", mct@mc_id)
    }
...
#order MCs by type, mean age
  if(is.null(mc_ord)) {
    ...
    names(mc_rank) = as.character(1:length(mc_rank))
    clust_border_y = sort(as.numeric(unlist(
                            lapply(tapply(seq_along(clust_vec), clust_vec, identity)[unique(as.character(clust_vec))], function(x) head(x,1))
                                        )
                            		)
							)
  }
...
if(0) {
    ...
    max_lfp = 3
    min_lfp = -max_lfp
    x = pmin(pmax(log2(mc@mc_fp[g, ]), min_lfp), max_lfp) - min_lfp
	shades = colorRampPalette(colspec)(100 * (max_lfp-min_lfp) + 1)
	if (is.null(filt_mcs)) {gene_col = shades[round(100 * x) + 1]}
	else {gene_col = ifelse(1:length(x) %in% filt_mcs, shades[round(100 * x) + 1], 'white')}

	if(is.null(propogate) & is.null(mc_t_score)) {
		par(cex.main=4, col.main='black')
		plot(c(x1,x2), c(y1,y2), pch=19, col=gene_col[c(nn$mc1,nn$mc2)],cex=mc_cex, main=g)
		mc_rgb = col2rgb(gene_col)/256
		f = nn$mc1>0 & nn$mc2 > 0
...
#		points(c(x1,x2), c(y1,y2), pch=19, col=mc@colors[c(nn$mc1,nn$mc2)],cex=1)
		...
		rect(seg_df$x1-0.12,seg_df$y1-0.5, seg_df$x1+0.12, seg_df$y1+0.5,
					col = gene_col[nn$mc1], border=NA)
		rect(seg_df$x2-0.12,seg_df$y2-0.5, seg_df$x2+0.12, seg_df$y2+0.5,
					col = gene_col[nn$mc2], border=NA)
	} else {
		par(cex.main=4, col.main='black')
		plot(c(x1,x2), c(y1,y2), pch=19, col=mc@colors[c(nn$mc1,nn$mc2)],cex=mc_cex, main=g)
		...
		points(c(x1,x2), c(y1,y2), pch=19, col=gene_col[c(nn$mc1,nn$mc2)],cex=m_cex)
	}
...
#	  text(c(x1[f1],x2[f2]),c(y1[f1],y2[f2]),labels = c(nn$mc1[f1],nn$mc2[f2]), cex=1)
    ...
    xc = matrix(c(
        rep(0,length(clust_border_y)), 
        rep(max(nn$time2), length(clust_border_y))
        ), nrow = length(clust_border_y), ncol = 2)
    yc = matrix(rep(clust_border_y, 2), nrow = length(clust_border_y), ncol = 2)
    sapply(seq_along(1:nrow(yc)), function(i) lines(xc[i,], yc[i,], lty = 'dashed', lwd = 2))
	mids = as.numeric(c(sapply(2:length(clust_border_y), function(cby, i) {round(mean(c(cby[[i-1]],cby[[i]])))}, cby = clust_border_y),
				max(as.numeric(nn$mc2))))
	print(mids)
	text(rep(0,length(mids)),mids,unique(clust_vec), cex = 3)
	dev.off()
}
