...
jaspar_tracks = gtrack.ls('jaspar')
...
tfoi = c('Bcl11b', read.delim('./data/tfoi.txt') %>% unlist %>% sort %>% as.character)
...
# }
    ...
    readr::write_excel_csv(motif_c, file.path(output_dir, glue::glue('motif_vals_{n_peaks}.csv')))
    ...
    readr::write_excel_csv(motif_c, file.path(output_dir, glue::glue('motif_enrich_all_{n_peaks}.csv')))
