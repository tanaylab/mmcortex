...
promo_peaks = promo_peaks[!(duplicated(dimnames(promo_peaks)[[1]])),]

m = Matrix::rowMeans(promo_peaks)
s = apply(promo_peaks,1, sd)


plot(log2(m), log2(s**2/m))

# utr3 = gintervals.load('utr3')
# utr5 = gintervals.load('utr5')

# tss = gintervals.load('tss')

# genes_int = intersect(tss$geneSymbol, intersect(utr5$geneSymbol, utr3$geneSymbol))
# # utr3 = filter(utr3, utr3$geneSymbol %in% genes_int) %>% filter(!duplicated('geneSymbol'))
# utr3 = utr3[utr3$geneSymbol %in% genes_int,]; utr3 = utr3[!duplicated(utr3$geneSymbol),]
# utr5 = utr5[utr5$geneSymbol %in% genes_int,]; utr5 = utr5[!duplicated(utr5$geneSymbol),]
# tss = tss[tss$geneSymbol %in% genes_int,]; tss = tss[!duplicated(tss$geneSymbol),]

# cols_j = c('chrom', 'start', 'end', 'geneSymbol')
# gene_coords = left_join(utr5[,cols_j], utr3[,c(cols_j, 'strand')], by = 'geneSymbol')

# gene_coords = gene_coords %>% mutate(len = abs(start.y - start.x)) %>% mutate(len_cor = len - 1500)

gb_peaks = readRDS(file.path(wd, 'scatac_data/genebody_counts.RDS'))

ig_peaks = readRDS(file.path(wd, 'scatac_data/peak_counts.RDS'))

# qs_atac = data.frame(cbind(
#     quantile(Matrix::colSums(promo_peaks), probs = c(seq(0.05, 0.95, 0.05), seq(0.96, 0.98, 0.01), seq(0.99, 1, 0.0025))),
#     quantile(Matrix::colSums(gb_peaks), probs = c(seq(0.05, 0.95, 0.05), seq(0.96, 0.98, 0.01), seq(0.99, 1, 0.0025))),
#     quantile(Matrix::colSums(ig_peaks), probs = c(seq(0.05, 0.95, 0.05), seq(0.96, 0.98, 0.01), seq(0.99, 1, 0.0025)))
#     ))
# colnames(qs_atac) = c('prom', 'gene_body', 'intergenic')

# rownames(qs_atac) = names(quantile(1, probs = c(seq(0.05, 0.95, 0.05), seq(0.96, 0.98, 0.01), seq(0.99, 1, 0.0025))))
...
cells_ig = colnames(ig_peaks)[csi >= 9000 & csi <= quantile(csi, q_max)]
cells_gb = colnames(gb_peaks)[csg >= 9000 & csg <= quantile(csg, q_max)]
cells_prom = colnames(promo_peaks)[csp >= 3000 & csp <= quantile(csp, q_max)]
...
length(intersect(cells_gb, intersect(cells_ig, cells_prom)))
