...
num_seqs_to_take = apply(diff_enh_per_st, 2, function(x) {x[x > 1250] = 1250; return(as.list(x))})
...
hi_var_tad = unlist(lapply(head(levels(xcut), -6), function(q) names(tad_min)[which(xcut == q & tad_max >= quantile(tad_max[xcut == q], 0.75))]))

const_lo_tad = unlist(lapply(levels(xcut)[1:11], function(q) names(tad_min)[which(xcut == q & tad_max <= quantile(tad_max[xcut == q], 0.25))]))
const_hi_tad = unlist(lapply(levels(xcut)[16:18], function(q) names(tad_min)[which(xcut == q & tad_max <= quantile(tad_max[xcut == q], 0.6))]))
