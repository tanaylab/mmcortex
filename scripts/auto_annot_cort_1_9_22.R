...
mc2d_path <- './figs/pl_cort_test_reannot_st_1-9-22.png'
flow_plot_path <- './figs/pl_cort_flow.png'
flow_plot_w_id_path <- './figs/pl_cort_flow_mc_id.png'
...
mc_module_score <- t(sapply(gene_modules, function(mi) colSums(mc@mc_fp[mi,])))

st[which(is.na(st))] <- rownames(mc_module_score)[apply(mc_module_score[,which(is.na(st))], 2, which.max)]
...
icpn_cfupn_inds <- intersect(mcs_that_flow_to_CPN, mcs_that_flow_to_CfuPN)
icpn_late_inds <- setdiff(mcs_that_flow_to_CPN, union(which(st %in% cpn), icpn_cfupn_inds))
bad_icfupn_inds <- which(st == 'iCfuPN')[which(st == 'iCfuPN') %in% union(icpn_cfupn_inds, icpn_late_inds)]
st[bad_icfupn_inds] <- 'iCPN/CfuPN'
...
mcmd_old <- vroom::vroom('./BonevCollab//mcmd_pl_cort_freeze_1_9_22.tsv')
color_key <- unique(mcmd_old[,c('st', 'color')])
col_annot <- as.data.frame(as.matrix(mcmd_old$st))
...
scdb_add_mc(id = nm_test, mc2)
mct@mc_id <- nm_test
scdb_add_mctnetwork(id = nm_test, mct)
mcf@net_id <- nm_test
scdb_add_mctnetflow(nm_test, mcf)

mctnetwork_plot_net(mct_id = nm_test, 
                    flow_id = nm_test,
                    fn = './figs/pl_cort_flow_test_reannot_st_1-9-22.png', 
                    flow_thresh = min(mcf@edge_flows[mcf@edge_flows > 0]), 
                    mc_ord = cust_mc_ord_st_new, plot_mc_ids = F,h = 2000, w = 2000)

mcell_mc2d_plot(nm, colors = ann_colors$st_new[col_annot$st_new], fig_fn = mc2d_path)
