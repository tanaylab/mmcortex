mctnetwork_plot_gene = function(mct_id, gene, fn, 
...
#     l = 100
#     legc = mc@e_gc[g,] + 1e-06
#     legc = log2(legc/median(legc))
#     min_val = min(legc)
#     max_val = max(legc)
#     rng = max_val - min_val
#     clrmp = colorRampPalette(c('blue3', 'white', 'yellow', 'red3'))(l)
#     lc = length(clrmp)
#     brks = c(seq(min_val, 0, length.out = l/(lc-1)), # blue to white
#              seq(0+rng/l,max_val/2,length.out = round(l/(lc-1))), # white to yellow
#              seq(max_val/2+rng/l,max_val,length.out = round(l/(lc-1)+1)) # yellow to red
#             )
