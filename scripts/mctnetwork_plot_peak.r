#' Plot an McPeak across a flow model
#'
#' @param mct_id an mctnetwork object (metacell1)
#' @param peak_name peak name as obtained from McPeaks@peaks$peak_name
#' @param fn output file name
#' @param mcpeaks (optional) an McPeaks object (see McATAC documentation)
#' @param mcpeaks_path (optional) path to McPeaks object
#' @param mc_ord (optional) an ordered vector of length \code{ncol(mcpeaks@egc)}, ordering the metacells for plotting
#' @param colors_ordered (optional) whether to sort metacells based on color (colors retreived from \code{mct@mc_id} where mct is the mctnetwork with \code{mct_id})
#' @export

mctnetwork_plot_peak = function(mct_id, peak_name, fn, 
                        mcpeaks = NULL, mcpeaks_path = NULL,
    ...
    if (is.null(mcpeaks)) {
        if (is.null(mcpeaks_path)) {
            cli_abort("Either {.var mcpeaks} or {.arg mcpeaks_path} must be provided")
        } else {
            mcpeaks = readRDS(mcpeaks_path)
        }
    }
	if(is.null(mct)) {
    ...
    if (!(g %in% mcpeaks@peaks$peak_name)) {
        cli_abort("cannot find peak ", g, " in {var. peaks} of McPeaks {.}", mct@mc_id)
    }
...
#order MCs by type, mean age
    ...
    max_lfp = 3
    min_lfp = -max_lfp
    x = pmin(pmax(mcpeaks@fp[g, ], min_lfp), max_lfp) - min_lfp
	shades = colorRampPalette(colspec)(100 * (max_lfp-min_lfp) + 1)
	gene_col = shades[round(100 * x) + 1] 
	if(is.null(propogate) & is.null(mc_t_score)) {
		par(cex.main = 3)
