...
png('./figs/pl_atac_prom_feat_select.png', h=1000,w=1000,r=150)
...
#     cor_mat = tgs_cor(as.matrix(mat[feats_filt,]), as.matrix(mc_rna@mc_fp[feats_filt,]), spearman = T)
    cor_mat = tgs_cor(as.matrix(mat[feats_filt,]), log2(1e-07 + mc_rna@e_gc[feats_filt,]), spearman = T)
    cor_km = tglkmeans::TGL_kmeans(df = cor_mat, k = K, seed = SEED)
    sc_mic_cl = data.frame(t(tgs_matrix_tapply(as.matrix(mat), cor_km$cluster, mean)))
#     cor_mic_cl = tgs_cor(x = as.matrix(sc_mic_cl[feats_filt,]), y = mc_rna@mc_fp[feats_filt,], spearman = T)
    cor_mic_cl = tgs_cor(x = as.matrix(sc_mic_cl[feats_filt,]), y = log2(1e-07 + mc_rna@e_gc[feats_filt,]), spearman = T)
...
plot_mc_pheatmap = function(cor_km, cor_mat, cor_mic_cl, day) {
                pheatmap::pheatmap(cor_mic_cl[,cust_mc_ord_st], cluster_cols = F, cluster_rows = F, 
